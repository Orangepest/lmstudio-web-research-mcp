...
def research_entries(research_dir: Path) -> dict[str, dict[str, Any]]:
    return {
        'chrome-mcp-server': {
...
def merge_config(existing: dict[str, Any], *, research_dir: Path) -> dict[str, Any]:
    data = dict(existing)
    servers = dict(data.get('mcpServers') or {})
    servers.update(research_entries(research_dir))
...
def main() -> int:
    ...
    print(json.dumps(merge_config(existing, research_dir=research_dir), indent=2))
    return 0
